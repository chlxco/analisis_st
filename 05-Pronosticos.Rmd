...
RED ELMAN:
```{r message=FALSE, warning=FALSE}
# Instalar y cargar la librería RSNNS
...
df <- data.frame(Tiempo = time,
                 Valor = indice.ts,
                 Lag1 = lag(indice.ts, 1),
                 Lag2 = lag(indice.ts, 2),
                 Lag3 = lag(indice.ts, 3))

# Convertir los datos de entrada a formato de matriz
input_data <- as.matrix(df[, c("Lag1", "Lag2", "Lag3")])
output_data <- as.matrix(df[, "Valor"])

# Definir la arquitectura de la red Elman
elman_model <- elman(input_data, output_data, hidden = 3, learnFunc = "Std_Backpropagation", 
                     maxit = 1000, linear.output = TRUE)

# Realizar el pronóstico con la red Elman
elman_forecast <- predict(elman_model, input_data)

# Graficar el pronóstico
plot(indice.ts, type = "l", col = "#00a0dc", main = "Pronóstico con Red Elman")
lines(elman_forecast, col = "red")
...
# Definir la arquitectura de la red Jordan
jordan_model <- jordan(input_data, output_data, hidden_units = 3, activation = "tanh")

# Realizar el pronóstico con la red Jordan
jordan_forecast <- predict(jordan_model, input_data)

# Graficar el pronóstico
plot(indice.ts, type = "l", col = "#00a0dc", main = "Pronóstico con Red Jordan")
lines(jordan_forecast, col = "red")
